...
if __name__ == '__main__':
    ...
    if evaled_stations:
        try:
            ...
            sta.sendStationCount(len(evaled_stations), datetime.fromisoformat(max(evaled_stations, key=lambda station: station[3])[3]))
        except error.HTTPError as e:
            logging.error("Request to SensorThings API (STA) aborted due to '%s'", e)
    else:
